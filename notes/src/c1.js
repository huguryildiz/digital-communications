...
{t:'p', text:'The coefficient is the Fourier-series coefficient of $p(t)$, and it is the same for every harmonic: $a_k=\\frac{1}{T_s}\\int_{-T_s/2}^{T_s/2}\\delta(t)e^{-j2\\pi kf_0t}\\,dt=\\frac{1}{T_s}$, by the sifting property. Now convolve $G$ with that train, one impulse at a time:'},
{t:'eq', tex:'G_\\delta(f)=G(f)*P(f)=\\frac{1}{T_s}\\sum_{n=-\\infty}^{\\infty}G(f)*\\delta(f-nf_s)'},
{t:'p', text:'A convolution with a shifted impulse only shifts the function. Write the convolution integral out, take the minus sign outside the argument of the impulse, and use that the impulse is even, $\\delta(-u)=\\delta(u)$:'},
...
{t:'p', text:'The last line is the sifting property: the integral returns the value of $G$ at the point where the impulse stands. Each impulse in $P(f)$ therefore places one copy of $G$ at its own multiple of $f_s$, and the sum of those copies is the result the rest of the chapter rests on.'},
...
{t:'p', text:'Filtering in frequency is convolution in time, so the reconstructed signal is the impulse train convolved with $h_{\\mathrm{LPF}}$. Write that convolution as one integral in the dummy variable $\\tau$:'},
{t:'eqbox', cap:'The convolution integral, and the sifting step', tex:[
 ...
 after:'The second line makes two moves. The sum does not depend on $\\tau$, so it comes outside the integral; and what is left inside is a sinc against a shifted impulse, which the sifting property replaces by the value of the sinc at $\\tau=nT_s$. That value is the interpolation formula below.'},
