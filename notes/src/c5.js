...
{t:'box', kind:'def', hd:'An IQ modem from samples', html:'Build a QPSK and 16-QAM modem from sampled carriers and measure how often it is wrong.<br><b>Practises:</b> The IQ modulator as two products and a sum. Two correlators as sums over samples. Counting errors to estimate $P_e$.<ol><li>Build a cosine and a sine carrier over one symbol at $64$ samples.</li><li>Map random bits to QPSK points and form $s(t)=I\\cos-Q\\sin$.</li><li>Add Gaussian noise, correlate, and pick the nearest point.</li><li>Repeat for 16-QAM and plot both error rates against $E_b/N_0$.</li></ol><b>Look for:</b> the measured points follow the formulas of this chapter once enough errors are counted.'},
{t:'box', kind:'def', hd:'Gray against natural labels', html:'Measure how much Gray labels lower the bit error of 8-PSK.<br><b>Practises:</b> Gray code as $i\\oplus\\lfloor i/2\\rfloor$. Why a symbol error usually lands on a neighbour. Bit error against symbol error.<ol><li>Send random 8-PSK symbols over noise.</li><li>Label the points once with natural binary and once with Gray code.</li><li>Count the bit errors of each on the same noisy symbols.</li><li>Plot both bit errors and $P_e/3$ against $E_b/N_0$.</li></ol><b>Look for:</b> the Gray curve sits on $P_e/3$. The natural curve sits above it.'},
{t:'box', kind:'def', hd:'DPSK with a drifting phase', html:'See why differential detection survives a slowly drifting carrier phase.<br><b>Practises:</b> Differential encoding of bits into phase changes. Coherent against differential detection. What a phase drift does to each.<ol><li>Encode random bits as binary DPSK.</li><li>Turn each received symbol by a phase that grows slowly with time.</li><li>Detect coherently with a fixed reference and differentially.</li><li>Count the errors of each as the drift rate grows.</li></ol><b>Look for:</b> the coherent receiver fails once the drift passes $90^{\\circ}$. The differential one barely notices.'},
{t:'box', kind:'def', hd:'An adaptive link', html:'Build a link that changes its constellation as the SNR changes.<br><b>Practises:</b> Thresholds for a target bit error. Throughput against a fixed scheme. Why a margin above each threshold helps.<ol><li>Draw an SNR that wanders slowly between $0$ and $35$ dB.</li><li>At each block pick the largest set that meets the target.</li><li>Simulate the block and count its errors.</li><li>Compare the bits delivered with a fixed QPSK link.</li></ol><b>Look for:</b> the adaptive link carries several times the bits at about the same error rate.'}
];
})();
