...
Object.assign(LABS, (function(){
  ...
  const C = (() => {
    ...
    /* The matched filter output is the correlation of the pulse with itself,
       ...
       alone — neither the noise, nor the sampling instant, nor the phase of
       the animation moves it — so it is computed once per shape and reused
       across all forty-one phases. Recomputing it every frame would be a
       quarter of a million products spent to redraw one marker. */
    ...
    /* Act two of the animation drives the sampling instant. The slider is a
       ...
       phase itself moves — never inside `draw`, which would snap the slider
       back under the reader's finger. Moving the thumb as well as the value
       shows which control the animation is driving.
       At phase 21 this gives 20 % of T, and at phase 40 exactly 100 %, which
       is the resting instant the laboratory rests at today. */
    function scan(root){
    ...
    function draw(root){
      ...
      /* Act one sweeps the filter across the pulse and builds the output curve
         as far as the sweep has reached; act two leaves the curve whole and
         walks the sample to the peak. */
      ...
      if(sweeping){
        ...
        /* The overlap is what the integral is accumulating at this instant,
           and shading it is the whole reason the sweep is worth watching.
           Amber is the filter's colour and `dec.h` is its low-opacity fill,
           which carries a separate value for the dark theme. */
      ...
      /* The curve is the running integral of that shaded overlap, so during
         act one it exists only as far as the sweep has carried it. */
