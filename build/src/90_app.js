...
def _ss_figs():
  ...
  function draw(){
    /* A figure is drawn on its own grid — 480 units wide for a typical one —
       ...
       A slide, and the laboratory on one, draws with the back-row emphasis;
       a scene not yet converted keeps the scale it was fitted at. */
  ...
  function fitScene(){
    ...
    /* The eyebrow and the title are exempt from the scale: `--hk` = 1/k sizes
       them up by what the transform takes away, so every slide title reads at
       the same size whether its scene fits or not. A scene not yet converted
       to a slide scales its title with the rest, as it was fitted. */
    ...
    for(let pass=0; pass<8; pass++){
      ...
      if(host.classList.contains('slide')) inner.style.setProperty('--hk', (1 / k).toFixed(4));
