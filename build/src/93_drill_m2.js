...
CONTENT.DRILLTYPES.M2 = [
  ...
  { k:'nongauss', name:'A binary decision under a noise density that is not Gaussian',
    ...
    method:['Find the constant from $\\int f_N(n)\\,dn=1$ before anything else.',
            ...
            'The threshold that minimises the error still satisfies $P(s_1)f_Y(\\lambda\\mid s_1)=P(s_0)f_Y(\\lambda\\mid s_0)$. However, with a density that has finite support or a kink the solution may sit at an edge rather than where a derivative vanishes.'],
    go:'m2-stat' },
...
{ id:'D2-01', module:'M2', type:'mf', src:'MT Q3',
  stem:'A rectangular pulse has amplitude $A=2$ V and duration $T=4$ ms. It is received in white Gaussian noise with two-sided density $N_0/2$, where $N_0=10^{-3}$ W/Hz.',
  ...
  sol:'<b>Given.</b> $A=2$ V, $T=4$ ms, $N_0=10^{-3}$ W/Hz.<br>'
     ...
     +'<b>Check.</b> The filter output is a triangle peaking at $t=T$ with peak equal to the energy. Therefore, the value in (b) is also the height of that triangle. Halving the amplitude quarters the energy and costs $6.02$ dB. This is the same exchange rate the quantizer had for a different reason.',
  err:'Reporting $y(T)=A=2$. The matched filter returns the <em>energy</em>, not the amplitude, and the two have different units.',
  teach:'Ask what happens to (c) if the pulse is stretched to $8$ ms at the same amplitude. The energy doubles and the ratio gains $3$ dB. At the cost of halving the bit rate. This is the trade the whole subject turns on.' },
...
{ id:'D2-02', module:'M2', type:'mf', src:'MT Q3',
  ...
  sol:'<b>Given.</b> A ramp reaching $A$ at $t=T$.<br>'
     ...
     +'<b>Check.</b> Both filters are triangles reversed. The difference is entirely in the energy. A pulse of the same <em>energy</em> and any shape would lose nothing at all. The comparison here is at equal peak amplitude. This is a constraint on the transmitter rather than on the theory.',
  err:'Concluding that the ramp is worse only because of its shape. It carries less energy for the same peak amplitude. With an average-power limit, equal-energy pulses give equal matched-filter performance.',
...
{ id:'D2-03', module:'M2', type:'mf', src:'CH8 s.13',
  ...
  sol:'<b>Given.</b> A half-sine of peak $B$ and a rectangle of amplitude $A$, both of duration $T$.<br>'
     ...
     +'<b>Check.</b> The half-sine must peak $41\\%$ higher because its average square is half its peak square. For a rectangle, these values are equal. The bound $2E/N_0$ contains no shape parameter. Therefore, equal-energy pulses give equal output ratios.',
...
{ id:'D2-04', module:'M2', type:'ebn0', src:'Final Q2',
  ...
  sol:'<b>Given.</b> $E_b=4\\times10^{-6}$ J, $N_0=10^{-6}$ W/Hz, equal priors.<br>'
     ...
     +'<b>Check.</b> About one bit in $430$. At $6$ dB the system is in the region where a decibel is worth roughly a factor of three in error rate. This is the shape of the curve near its knee. Further out each decibel is worth an order of magnitude.',
...
{ id:'D2-05', module:'M2', type:'ebn0', src:'Final Q2',
  ...
  sol:'<b>Given.</b> A target $P_b=10^{-5}$.<br>'
     ...
     +'<b>Check.</b> The previous question gave $2.34\\times10^{-3}$ at $6.02$ dB. Moving from there to $10^{-5}$ has cost $3.57$ dB and bought a factor of $234$. Roughly two orders of magnitude for three and a half decibels. This is the steepness the curve has in this region.',
...
{ id:'D2-08', module:'M2', type:'nongauss', src:'MT Q4',
  ...
  sol:'<b>Given.</b> Laplacian noise with unit scale; $s_m=\\pm1$; threshold at zero.<br>'
     ...
     +'<b>Check.</b> The Laplacian noise gives a smaller error here. It has more probability near zero than a Gaussian with the same variance. Its remaining probability lies farther into tails that rarely cross the thresholds at $\\pm1$.',
  ...
  teach:'Ask what the optimal threshold is here. By symmetry it is zero. However, the derivation that gives it is the density-crossing condition and not the Gaussian formula. A distinction the next question makes sharper.' },
...
{ id:'D2-09', module:'M2', type:'isi', src:'CH8 s.44–47',
  ...
  sol:'<b>Given.</b> $R_b=20$ kbit/s, $\\alpha=0.35$.<br>'
     ...
     +'<b>Check.</b> At $\\alpha=0$ the answer would be $10$ kHz and the pulse would be the ideal Nyquist one. At $\\alpha=1$ it would be $20$ kHz. Every value of $\\alpha$ between gives a system with zero interference at the sampling instants. Therefore, the $3.5$ kHz is bought entirely for tolerance to timing error and ease of filtering.',
...
{ id:'D2-10', module:'M2', type:'isi', src:'CH8 s.44–47',
  ...
  sol:'<b>Given.</b> $B_T=24$ kHz available, $\\alpha=0.5$.<br>'
     ...
     +'<b>Check.</b> With $\\alpha=0$, the same channel carries $48$ kbit/s. The roll-off reduces the rate by one third. This result is the effect of $50\\%$ excess bandwidth. Practical systems commonly use $0.2\\le\\alpha\\le0.5$.',
...
{ id:'D2-11', module:'M2', type:'isi', src:'CH8 s.42–44',
  ...
  sol:'<b>Given.</b> $p(t)=\\operatorname{sinc}(2Wt)$, $T_b=1/(2W)$.<br>'
     ...
     +'<b>Check.</b> The two conditions are equivalent. Constant spectral tiling gives a sampled pulse that contains one impulse. Therefore, the continuous pulse is zero at every other sampling instant.',
  err:'Checking only that $p(0)=1$. The criterion also requires zeros at all other sampling instants. A correct peak with incorrect zeros causes intersymbol interference.',
...
{ id:'D2-12', module:'M2', type:'cond', src:'MT Q3',
  ...
  sol:'<b>Given.</b> White Gaussian noise of two-sided density $N_0/2$. A basis function of unit energy.<br>'
     ...
     +'<b>Check.</b> The answer does not contain $T_b$, and it should not. Lengthening the interval spreads the same unit of energy over more time. Therefore, $\\psi$ falls and the two effects cancel exactly. The unit-energy normalisation is what makes that happen.',
  err:'Reporting $N_0$ rather than $N_0/2$, by taking the density as one-sided. Every error probability in this course then comes out $\\sqrt2$ too optimistic inside the $Q$. This is a $3$ dB error and is invisible in the algebra.',
...
{ id:'D2-13', module:'M2', type:'full', src:'MT Q3',
  stem:'An additive white Gaussian noise channel has two-sided power spectral density $9$ W/Hz. Two equiprobable messages use $s_0(t)=-3$ V and $s_1(t)=+3$ V for $0\\le t\\le T_b=2$ s. A matched-filter demodulator uses the unit-energy rectangular basis $\\psi(t)=1/\\sqrt{T_b}$.',
  parts:['Give the impulse response of the matched filter.',
  ...
  sol:'<b>Given.</b> $A=3$ V, $T_b=2$ s, $N_0/2=9$ so $N_0=18$ W/Hz, equal priors.<br>'
     ...
     +'<b>Check.</b> Approximately one bit in thirteen is wrong. The ratio $E_b/N_0=1$ equals $0$ dB. The signal separation is $8.49$, and the noise standard deviation is $3$. The tail beyond half of this separation is $Q(1.414)$.',
  ...
  teach:'Part (b) requires the complete density formulas. Each formula must include its mean and variance. This form also exposes an incorrect noise-density convention.' },
...
{ id:'D2-14', module:'M2', type:'full', src:'MT Q3',
  ...
  sol:'<b>Given.</b> On-off signalling with equal priors.<br>'
     ...
     +'<b>Check.</b> These signal points are $\\sqrt{2E_b}$ apart. Antipodal points are $2\\sqrt{E_b}$ apart, which is larger by $\\sqrt2$. Error probability depends on this separation. The distance ratio gives the $3$ dB difference. Module 4 develops this result for general constellations.',
  err:'Taking $E_b=A^{2}T_b$. Half the bits carry no energy at all. Therefore, the average is half that, and reporting the peak energy as $E_b$ makes on-off look as good as antipodal.',
...
{ id:'D2-15', module:'M2', type:'full', src:'MT Q4',
  ...
  teach:'Part (d) uses non-Gaussian noise. Therefore, the antipodal Gaussian formula does not apply. Use the given density to calculate the conditional probabilities directly.' },

{ id:'D2-16', module:'M2', type:'full', src:'MT Q4',
  stem:'In a binary system, $Y=-2+N$ for symbol "0" and $Y=+2+N$ for symbol "1". The noise $N$ is exponential with mean $4$: $$f_N(n)=\\tfrac14 e^{-n/4},\\qquad n\\ge0.$$The density is zero for $n<0$. The symbols are equally likely. The detector selects "1" when $Y>\\lambda$.',
  ...
  sol:'<b>Given.</b> One-sided exponential noise of mean $4$; signal points at $\\pm2$; equal priors.<br>'
     ...
     +'<b>Solution (d).</b> The optimum is at a support boundary, not at a stationary point. One conditional error is zero at this boundary. Gaussian noise cannot give this result because its density is positive everywhere.<br>'
...
{ id:'D2-17', module:'M2', type:'full', src:'Final Q2',
  ...
  sol:'<b>Given.</b> $E_b=2$, $N_0=0.4$, $P(s_1)=0.25$.<br>'
     ...
     +'<b>Check.</b> The threshold moved towards the less likely symbol. Therefore, the error given $s_1$ went up and the error given $s_0$ went down. The weighted total fell. This is the only test that matters. Notice that the larger conditional error is the one carrying the smaller weight. That is not a coincidence but the condition being satisfied.',
...
{ id:'D2-19', module:'M2', type:'full', src:'CH8 s.44–48',
  ...
  sol:'<b>Given.</b> $R_b=64$ kbit/s, $48$ kHz available.<br>'
     ...
     +'<b>Solution (d).</b> At $\\alpha=0$ the system would occupy only $32$ kHz and still meet the criterion. Therefore, it fits comfortably. What goes wrong is everything else. The spectrum has vertical edges and cannot be built, and the pulse decays as $1/t$. Therefore, a small timing error lets a long tail of neighbours contribute at once and the interference does not settle. Using the whole channel buys a pulse decaying as $1/t^{3}$ and a filter that can be made.<br>'
...
{ id:'D2-20', module:'M2', type:'full', src:'CH8 s.35–41',
  ...
  sol:'<b>Given.</b> $p(0)=1$, $p(\\pm T_b)=0.10$, $p(\\pm2T_b)=-0.04$, $\\sigma=0.15$.<br>'
     ...
     +'<b>Check.</b> The estimate is a worst case and therefore pessimistic. The pattern that puts every neighbour against the wanted symbol occurs one time in sixteen here, and the average error rate is better than $7.9\\times10^{-7}$. It is the right number to design with. This occurs because the bad pattern will occur.',
...
{ id:'m2-drill', module:'M2', nav:'Module 2 · practice questions',
  ...
  {t:'small', html:'Work each question before opening its solution. Use these checks:<ul><li>The matched-filter sample equals an energy.</li><li>The decision-noise variance is $N_0/2$.</li><li>The optimal threshold moves away from the more likely signal.</li><li>Error probability decreases as $E_b/N_0$ increases.</li></ul>'},
  {t:'rule', short:true},
  {t:'drill', module:'M2'}
]}
