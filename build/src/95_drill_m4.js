...
{ id:'D4-01', module:'M4', type:'rule', src:'CH9 s.34',
  ...
  sol:'<b>Given.</b> One observation and four equally likely points.<br>'
     ...
     +'<b>Check.</b> The observation has a positive first coordinate and a negative second. Therefore, it lies in the fourth quadrant. For this constellation the four regions <em>are</em> the four quadrants. The arithmetic and the picture agree, and for this constellation the picture is faster.',
...
{ id:'D4-04', module:'M4', type:'pairwise', src:'CH9 s.54',
  ...
  sol:'<b>Given.</b> Antipodal binary at $8$ dB.<br>'
     ...
     +'<b>Check.</b> The squared distance is halved, so the $Q$ argument decreases by $\\sqrt{2}$. This change gives the $3$ dB difference from Module 2.',
...
{ id:'D4-05', module:'M4', type:'union', src:'CH9 s.60',
  ...
  sol:'<b>Given.</b> $M=8$, $d_{\\min}=1.2$, $N_{\\min}=2$, $N_0=0.1$.<br>'
     ...
     +'<b>Solution (c).</b> Use the nearest-neighbor estimate. The minimum-distance bound treats all seven other points as if they were at $d_{\\min}$. Most points are farther away and contribute little. Thus, the bound is safe but $3.5$ times too large.<br>'
...
{ id:'D4-07', module:'M4', type:'design', src:'Final Q3',
  ...
  err:'Comparing them at equal <em>peak</em> energy instead of equal average energy. The four points of each constellation all have the same energy here. Therefore, the two happen to agree. However, in general the two normalisations give different answers.',
...
{ id:'D4-09', module:'M4', type:'regions', src:'CH9 s.50',
  ...
  sol:'<b>Given.</b> Three points on a line, equally likely.<br>'
     ...
     +'<b>Solution (b).</b> The middle one. It has a boundary on each side. Therefore, noise in either direction can carry it out of its region. In contrast, each outer point can only be lost in one direction. Its error probability is $2Q(1/\\sigma)$ against $Q(1/\\sigma)$ for the others.<br>'
...
{ id:'D4-14', module:'M4', type:'full', src:'Final Q3',
  ...
  sol:'<b>Given.</b> Four points, one of them far from the rest.<br>'
     ...
     +'<b>Check.</b> The nearest-neighbour form uses $d_{\\min}=2$ and $N_{\\min}=\\frac{2+1+1+0}{4}=1$, giving $2.34\\times10^{-3}$. Within one per cent of the full sum. This occurs because every other term is a hundred times smaller.',
  ...
  teach:'This constellation is deliberately badly designed. One symbol carries nine times the energy of the others and buys almost nothing. This occurs because the error rate is set by the three that are crowded together. It is the clearest argument for spending energy on $d_{\\min}$ rather than on any one point.' },
...
{ id:'D4-15', module:'M4', type:'full', src:'CH9 s.33, 49',
  ...
  sol:'<b>Given.</b> Antipodal binary, $E_b=1$, $N_0=0.25$, $P(\\mathbf{s}_0)=0.75$.<br>'
     ...
     +'<b>Check.</b> The optimal receiver makes the likely symbol safer and the unlikely one more exposed. $1.25$ against $4.22$ in units of $10^{-3}$. The weighted total still falls. This is the only test that matters. The gain is modest because the boundary moved only $3.4\\%$ of the separation.',
...
{ id:'D4-17', module:'M4', type:'full', src:'CH9 s.51, 56',
  ...
  sol:'<b>Given.</b> The general $M$-ary detection problem.<br>'
     ...
     +'<b>Solution (c).</b> $$P_e\\le\\frac{1}{M}\\sum_{k}\\sum_{j\\ne k}Q\\!\\left(\\sqrt{\\frac{d_{kj}^{2}}{2N_0}}\\right).$$ It is an inequality because the events overlap. An observation can be closer to two other points at once, and the sum counts it twice. In contrast, the true error counts it once.<br>'
     +'<b>Solution (d).</b> At low signal-to-noise ratio, the pairwise error regions overlap strongly. Their sum can then exceed one and gives a loose bound. At normal operating ratios, the pairwise terms and their overlaps are small. The bound is then close to the exact result.<br>'
...
{ id:'D4-18', module:'M4', type:'full', src:'Final Q3',
  ...
  sol:'<b>Given.</b> Four points equally spaced on a line, $E_s=1.25d^{2}$.<br>'
     ...
     +'<b>Solution (d).</b> At $12$ dB, $E_s/N_0=15.85$. Therefore, the ratio is $6.34$ and $Q(\\sqrt{6.34})=Q(2.518)=5.90\\times10^{-3}$, giving $P_e\\approx8.9\\times10^{-3}$. QPSK at the same $E_s/N_0$ has $d_{\\min}^{2}/2N_0=E_s/N_0=15.85$. Therefore, $P_e\\approx2Q(3.981)=6.9\\times10^{-5}$. Better by more than two orders of magnitude, at the same energy and the same two bits a symbol.<br>'
...
{ id:'D4-19', module:'M4', type:'full', src:'CH9 s.62–65',
  ...
  sol:'<b>Given.</b> Six points on a circle of radius $\\rho$ at $60^{\\circ}$ intervals.<br>'
     ...
     +'<b>Check.</b> The exact union bound would be $\\frac{1}{6}\\bigl[12Q(a)+12Q(a\\sqrt{3})+6Q(2a)\\bigr]$ with $a=\\sqrt{E_s/2N_0}$. This is $2Q(a)$ plus two terms that are smaller by orders of magnitude at any useful noise level. The nearest-neighbour form is the first term of that sum and the rest is what it discards.',
...
{ id:'D4-20', module:'M4', type:'full', src:'Final Q3',
  ...
  sol:'<b>Given.</b> Two four-point constellations at the same average energy.<br>'
     ...
     +'<b>Solution (c).</b> The squared distances are $2E_s$ and $0.8E_s$. Therefore, the $Q$ arguments differ by $\\sqrt{2/0.8}=1.58$. At high signal-to-noise ratio $Q$ falls faster than any power. Therefore, the ratio of the error probabilities grows without bound. At $E_s/N_0=12$ dB it is about $130$, and at $16$ dB about $4000$.<br>'
     +'<b>Solution (d).</b> Bandwidth, and simplicity. A one-dimensional constellation needs one basis function. Therefore, one carrier and one matched filter, and it occupies half the bandwidth of a two-dimensional scheme at the same symbol rate. QPSK buys its distance by using a second dimension, and that dimension is not free.<br>'
     +'<b>Check.</b> Both schemes carry two bits per symbol at energy $E_s$. Therefore, the comparison depends only on geometry. The two-dimensional constellation gives a smaller error rate because it has a larger minimum distance.',
...
{ id:'D4-21', module:'M4', type:'union', src:'CH9 s.60',
  ...
  sol:'<b>Given.</b> Three points on a line at $0$, $d$ and $2.2d$, equally likely, with $d^{2}/2N_0=9$.<br>'
     ...
     +'<b>Solution (c).</b> $Q(3)=1.3499\\times10^{-3}$ and $Q(3.6)=1.5911\\times10^{-4}$. Therefore, the bound is $\\frac{2}{3}(1.5090\\times10^{-3})=1.006\\times10^{-3}$. The nearest-neighbour form counts only the points at $d_{\\min}=d$. The left and middle points have one each, the right point has none. Therefore, $N_{\\min}=2/3$ and $P_e\\approx\\frac{2}{3}Q(3)=8.999\\times10^{-4}$.<br>'
     +'<b>Solution (d).</b> Use the bound. The approximation is $12\\%$ below it because it removes two boundaries at $1.2d$. The observation can cross these real boundaries. A design bound must not underestimate the error probability.<br>'
     +'<b>Check.</b> The minimum-distance bound gives $(M-1)Q(3)=2.700\\times10^{-3}$, comfortably above both, as a loose bound must be. The general union bound adds the outer pair at $2.2d$: $Q(6.6)=2.1\\times10^{-11}$, which changes nothing at four figures. So the intelligent bound has kept everything that mattered and dropped only what did not.',
...
{ id:'m4-drill', module:'M4', nav:'Module 4 · practice questions',
  ...
  {t:'small', html:'Work each question before opening its solution. Use these checks:<ul><li>$d_{\\min}$ is a nearest-neighbor distance.</li><li>$N_{\\min}$ is an average and can be noninteger.</li><li>A decision boundary lies between the two points that it separates.</li><li>More points at fixed energy reduce the minimum distance.</li></ul>'},
  {t:'rule', short:true},
  {t:'drill', module:'M4'}
]}
