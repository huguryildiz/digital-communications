/* ==========================================================================
   ...
   Ten tracks of clean waveforms, each degrading along its length — phase
   distortion, clipping, quantisation, and finally noise. It is the course's
   own chain seen end to end: a transmitted waveform, a channel that damages
   it, and a receiver left to decide what was sent.

   The code is the Signals and Systems cover's, line for line; only this
   comment differs. It sits behind the left of the pinned stage, masked out towards the
   figure and blended into the page by CSS (`#backdrop` in `index.html`).

   Changed from the original: the trace colours are the signal tints (cyan
   the clean signal, amber the channel at work, slate what is left when it
   has gone). The standalone page's postMessage parameter channel is gone.
