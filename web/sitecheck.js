...
(async () => {
  ...
  /* ---------------------------------------------------- the artifact ---- */
  {
    ...
    await page.goto(url('Digital_Communications.html'));
    ...
    const mods = await page.evaluate(() => [...new Set(CONTENT.DRILL.map(q => q.module))].sort());
    const total = await page.evaluate(() => CONTENT.DRILL.length);
    for (const m of mods) {
    ...
    if (pages < total) problems.push('only ' + pages + ' of ' + total + ' questions were reached');
  ...
  /* --------------------------------------------------- the four editions */
  /* The floor is per document, because the four differ in length; the
     formula reference is mostly typeset mathematics and therefore short in
     plain characters. */
  const FLOOR = {
    'Lecture_Notes.html': 100000,
    'Student_Workbook.html': 45000,
    'Formula_Reference.html': 12000,
    'Laboratory_Sheets.html': 18000
  };
