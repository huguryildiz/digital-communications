...
let art = fs.readFileSync(path.join(DIST, 'Digital_Communications.html'), 'utf8');
...
/* The floors sit a little under the counts at the time of writing, so a
   transform that silently stops matching is caught. No scene carries an
   instructor block yet; the transform is kept for the first one that does. */
...
if (teachFields < 100) fail('found only ' + teachFields + ' teaching notes');
...
log('Assembling site/');
fs.writeFileSync(path.join(SITE, 'Digital_Communications.html'), art);
log('  · Digital_Communications.html  ' + (art.length / 1048576).toFixed(2) + ' MB  (sanitised)');
...
/* The cover page, the hero background, Figure 1, and the cover and inside
   page of each PDF, rendered from the PDFs themselves. The label over each download
