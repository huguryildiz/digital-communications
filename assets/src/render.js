...
(async () => {
  ...
  try {
    ...
    for (const [name, size, scale] of outputs) {
      ...
      await page.setContent(`<html><head><style>*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;background:#04050f}body{display:grid;place-items:center}svg{display:block;width:${scale*100}%;height:${scale*100}%}svg .beam,svg .tail{display:none!important}</style></head><body>${svg}</body></html>`);
